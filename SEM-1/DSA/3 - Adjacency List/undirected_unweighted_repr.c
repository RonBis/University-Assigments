...
adj_list_node* create_node(int dest);
...
graph_list* input_graph() {
    ...
    for (int i = 0; i < E; i++) {
        ...
        // Add edge from->to (directed unweighted graph)
        adj_list_node* newNode = create_node(to);
        if (!newNode) {
            free_graph(G);
            return NULL;
        }
        ...
        // Add edge to->from (directed unweighted graph)
        adj_list_node* newNodeMirror = create_node(from);
        if (!newNodeMirror) {
            free_graph(G);
            return NULL;
        }
        // Insert at beginning of adjacency list
        newNodeMirror->next = G->adj_list[to].head;
        G->adj_list[to].head = newNodeMirror;
    }
